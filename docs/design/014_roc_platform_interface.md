...
> **状态: 🔮 Future Plugin (V3+)**
> **当前战略**: Pure Zig 元编程 (参见 D-015)
> **本文档定位**: Roc 作为**未来可选插件**，非核心路径
...
**2026-01-12 更新**: 经过深入分析，我们决定采用 **Pure Zig 元编程** 作为核心策略（详见 [D-015](015_pure_zig_metaprogramming.md)）。

**核心洞察**: Zig 的 `comptime` 能力足够强大，可以直接作为 DSL 使用，无需引入额外的语言抽象层。

**Roc 的新定位**:
- **V1-V2**: 不纳入，专注 Pure Zig 全平台支持
- **V3+**: 作为可选的"函数式插件"，为喜欢 TEA 架构的开发者提供选择
- **非必须**: Roc 不再是架构的必要组成部分
...
> **注意**: 本章节描述的是 Roc 集成的**远期愿景**，当前阶段应专注于 Pure Zig 实现（参见 D-015）。

Roc 作为**可选的函数式入口**，为特定用户群体提供价值：

```
┌───────────────────────────────────────────────────────────────────────────┐
│                      Roc Plugin 价值主张 (V3+ 可选)                        │
├───────────────────────────────────────────────────────────────────────────┤
│                                                                           │
│  适用场景:                                                                │
│  • 团队熟悉函数式编程 (Haskell/Elm/F# 背景)                              │
│  • 需要 TEA 架构处理复杂异步逻辑 (TON Actor 模型)                        │
│  • 偏好代数数据类型建模业务逻辑                                          │
│                                                                           │
│  不适用场景:                                                              │
│  • 需要极致性能控制 → 直接用 Pure Zig                                    │
│  • 团队无函数式背景 → 直接用 Pure Zig                                    │
│  • MVP 快速迭代 → 直接用 Pure Zig                                        │
│                                                                           │
│  当前建议:                                                                │
│  • V1-V2: 使用 Pure Zig + comptime (参见 D-015)                          │
│  • V3+: 评估 Roc 集成时机，根据社区需求决定                              │
│                                                                           │
└───────────────────────────────────────────────────────────────────────────┘
```

**核心战略调整**: Titan 的核心竞争力是 **Zig comptime 元编程**，而非语言层抽象。Roc 是"锦上添花"，不是"必须品"。
