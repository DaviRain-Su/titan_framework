...
│  │                                                                       │   │
│  │    ┌───────────────────────────────────────────────────────────┐    │   │
│  │    │           Zig comptime Metaprogramming Engine              │    │   │
│  │    │                                                            │    │   │
│  │    │   • @typeInfo 反射: 结构体 → 存储布局                      │    │   │
│  │    │   • inline for: 字段遍历 → SSTORE/Cell/AccountData         │    │   │
│  │    │   • comptime 分支: target == .evm/.ton/.solana             │    │   │
│  │    │   • comptimePrint: Zig → Yul/Fift 字符串生成               │    │   │
│  │    └───────────────────────────────────────────────────────────┘    │   │
│  │                              │                                       │   │
│  │                    ┌─────────┴─────────┐                             │   │
│  │                    │  路径选择器        │                             │   │
...
### 7.3 Pure Zig 元编程阶段 (当前重点)

| 任务 | 优先级 | 说明 |
| :--- | :---: | :--- |
| Storage(T) comptime 实现 | P0 | 结构体反射 → 多链存储布局 |
| Context 统一抽象 | P0 | sender/value/timestamp 跨链统一 |
| Router 入口点生成 | P0 | 方法选择器 → 分发逻辑 |
| Yul 代码生成器 | P0 | EVM 目标：Zig → Yul 字符串 |
| Fift 代码生成器 | P1 | TON 目标：Zig → Fift/Cell |

### 7.3.1 Roc 插件阶段 (V3+ 可选)

> **注意**: 以下任务为远期规划，当前阶段不纳入

| 任务 | 优先级 | 说明 |
| :--- | :---: | :--- |
| Roc AST 解析器集成 | P3 | 利用官方 Parser |
| Roc → Zig 代码生成器 | P3 | TEA 到 entrypoint 映射 |
| titan.roc Platform 接口 | P3 | Model/Msg/Cmd 类型定义 |
| TEA 异步调度器 | P3 | TON/Near 回调状态机 |
...
> **Titan OS = Pure Zig comptime 元编程 + LLVM 原生 (Solana/Wasm) + 转译 (TON/EVM/BTC) + ZK (Noir)**
>
> **Zig 即 DSL，代码即描述，降维打击，全链通吃。**

### 8.1 核心战略: Pure Zig Metaprogramming

**关键洞察**: Zig 的 `comptime` 能力足够强大，可以直接作为 DSL：
...
const MyContract = struct {
    ...
    pub fn transfer(self: *@This(), to: Address, amount: u256) !void {
        // comptime 自动根据 target 生成不同后端代码
...
// 一行代码，全链部署
pub const storage = titan.Storage(MyContract);
```

**三大抽象** (详见 D-015):
1. **Storage(T)**: 统一存储，comptime 反射生成多链布局
2. **Context**: 统一上下文，sender/value/timestamp 跨链一致
3. **Router**: 统一入口，自动生成方法分发逻辑
